...
load("./cv-results/ebf-current-10-all.RData")
fit.ebf.cur <- fit
load("./cv-results/ebf-future-10-all.RData")
fit.ebf.fut <- fit
load("./cv-results/gsk-current-10-all.RData")
fit.gsk.cur <- fit
load("./cv-results/gsk-future-10-all.RData")
fit.gsk.fut <- fit
...
load(paste("./cv-results/", method, "-future-10-all.RData", sep = ""))
...
load(paste("./cv-results/", method, "-current-10-all.RData", sep = ""))
beta.mu.int  <- apply(fit$beta.int[, , 1], 2, mean)
beta.mu.time <- apply(fit$beta.time[, , 1], 2, mean)
beta.ls.int  <- apply(fit$beta.int[, , 2], 2, mean)
beta.ls.time <- apply(fit$beta.time[, , 2], 2, mean)
...
panel <- gtable_cbind(g[[1]], g[[2]], size = "max")
...
plots <- list(plot.Delta.mu, plot.Delta.ls, plot.Delta.q90,
              plot.pdiff.mu, plot.pdiff.ls, plot.pdiff.q90)
g     <- lapply(plots, ggplotGrob)
row1 <- gtable_cbind(g[[1]], g[[2]], g[[3]], size = "max")
row2 <- gtable_cbind(g[[4]], g[[5]], g[[6]], size = "max")
panel <- gtable_rbind(row1, row2, size = "max")
...
plots <- list(plot.Delta.mu, plot.Delta.ls, plot.Delta.q90,
              plot.pdiff.mu, plot.pdiff.ls, plot.pdiff.q90)
g     <- lapply(plots, ggplotGrob)
col1 <- gtable_rbind(g[[1]], g[[2]], g[[3]], size = "max")
col2 <- gtable_rbind(g[[4]], g[[5]], g[[6]], size = "max")
panel <- gtable_cbind(col1, col2, size = "max")
